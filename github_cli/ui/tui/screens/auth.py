...
from github_cli.ui.auth.responsive_layout import ResponsiveAuthLayout, AuthLayoutConfig, AuthView
...
class AuthScreen(Screen[AuthResult]):
    ...
    def __init__(self, authenticator: Authenticator, layout_manager: ResponsiveLayoutManager | None = None) -> None:
        ...
        self.current_auth_view: AuthView | None = None
        self.device_code: str | None = None
        self.verification_uri: str | None = None

        logger.info(
            "Enhanced AuthScreen initialized with responsive capabilities")
    ...
    def compose(self) -> ComposeResult:
        ...
        self.current_auth_view = self.responsive_layout.create_auth_view(
            self.current_config)
        ...
        try:
            ...
            if self.error_handler:
                async with self.error_handler.auth_error_boundary("oauth_flow"):
                    ...
                    if self.progress_tracker:
                        self.progress_tracker.show_success(
            ...
            if self.progress_tracker:
                self.progress_tracker.show_error(
            ...
            auth_error = AuthenticationError(
                "Unexpected authentication error", cause=e)
            if self.error_handler:
    ...
    async def _run_auth_flow(self) -> AuthResult:
        ...
        try:
            ...
            self.verification_uri = device_flow_data.get(
            ...
            expires_in = device_flow_data.get(
    ...
    async def _open_browser_for_auth(self) -> None:
        ...
        try:
            if not self.verification_uri:
                logger.warning(
                    "No verification URI available for browser opening")
                return
            ...
            try:
                if webbrowser.open(self.verification_uri):
                    ...
                    logger.info(
                        "Successfully opened browser for authentication")
                    browser_opened = True
            except Exception as e:
            ...
            if not browser_opened:
                try:
                    ...
                    if browser_opened:
                        ...
                        logger.info(
                            "Successfully opened browser using platform-specific command")
                except Exception as e:
                    logger.debug(
                        f"Platform-specific browser command failed: {e}")
            ...
            if not browser_opened:
                ...
                try:
                    ...
                    logger.debug(
                        "pyperclip not available for clipboard support")
                except Exception as e:
    ...
    async def _request_device_code(self) -> dict:
        ...
        try:
            ...
            raise AuthenticationError(
                f"Failed to request device code: {e}") from e
    ...
    async def _validate_token(self, token_data: dict) -> dict:
        ...
        try:
            ...
            # Save the token
            saved_token = self.authenticator.token_manager.save_token(
                token_data)
            ...
            if user_info:
                logger.info(
                    f"Token validation successful for user: {user_info.login}")
                return {
                ...
                raise AuthenticationError(
                    "Token validation failed - unable to fetch user info")
    ...
    def _update_device_code_display(self) -> None:
        ...
        try:
            ...
            verification_url_widget = self.query_one(
                "#verification-url", Static)

            if self.current_config and self.current_config.layout_type == "compact":
                device_code_widget.update(
                    f"Code: {self.device_code or 'Loading...'}")
            else:
                device_code_widget.update(self.device_code or "Loading...")
                verification_url_widget.update(
                    f"URL: {self.verification_uri or 'Loading...'}")
    ...
    def _on_device_code_expired(self) -> None:
        ...
        if self.progress_tracker:
            self.progress_tracker.show_error(
                "Device code expired. Please retry authentication.")
        self._show_retry_option()

    def _on_layout_change(self, old_config: AuthLayoutConfig | None, new_config: AuthLayoutConfig) -> None:
    ...
    def cancel_auth(self) -> None:
        ...
        result = AuthResult(success=False, error=AuthenticationError(
            "Authentication cancelled by user"))
        self.dismiss(result)
