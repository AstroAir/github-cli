...
class SettingsWidget(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        with TabbedContent(id="settings-tabs"):
            ...
            with TabPane("General", id="general-tab"):
                with Container(id="general-settings", classes="settings-section"):
                    yield Static("General Settings", classes="settings-title")
                    
                    # Default repository view
                    with Horizontal(classes="setting-row"):
                        yield Label("Default Repository View:")
                        yield Select(
                            options=[
                                ("List View", "list"),
                                ("Grid View", "grid"),
                            ...
                            value=self.config.get("ui.default_repo_view", "list"),
                            id="default-repo-view"
                        )
                    
                    # Items per page
                    with Horizontal(classes="setting-row"):
                        yield Label("Items per Page:")
                        yield Select(
                            options=[
                                ("25", "25"),
                                ("50", "50"),
                            ...
                            value=str(self.config.get("ui.items_per_page", "50")),
                            id="items-per-page"
                        )
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Input(
                            value=str(self.config.get("ui.auto_refresh_interval", "60")),
                            id="auto-refresh-interval"
                        )
                    
                    # Enable auto-refresh
                    with Horizontal(classes="setting-row"):
                        yield Label("Enable Auto-refresh:")
                        yield Switch(
                            value=self.config.get("ui.auto_refresh_enabled", True),
                            id="auto-refresh-enabled"
                        )
                    
                    # Show tooltips
                    with Horizontal(classes="setting-row"):
                        yield Label("Show Tooltips:")
                        yield Switch(
            ...
            with TabPane("Appearance", id="appearance-tab"):
                with Container(id="appearance-settings", classes="settings-section"):
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Select(
                            ...
                            value=self.config.get("ui.color_scheme", "default"),
                            id="color-scheme"
                        )
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("ui.show_line_numbers", True),
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("ui.syntax_highlighting", True),
            ...
            with TabPane("API", id="api-tab"):
                with Container(id="api-settings", classes="settings-section"):
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("api.respect_rate_limits", True),
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Input(
                            value=self.config.get("api.base_url", "https://api.github.com"),
                            id="api-base-url",
            ...
            with TabPane("Notifications", id="notifications-tab"):
                with Container(id="notifications-settings", classes="settings-section"):
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("notifications.desktop_enabled", False),
                    ...
                    with Vertical(classes="checkbox-group"):
                        yield Checkbox(
                            ...
                            value=self.config.get("notifications.pr_updates", True),
                            id="notif-pr-updates"
                        )
                        yield Checkbox(
                            "Issue Updates",
                            value=self.config.get("notifications.issue_updates", True),
                            id="notif-issue-updates"
                        )
                        yield Checkbox(
                            "Review Requests",
                            value=self.config.get("notifications.review_requests", True),
                            id="notif-review-requests"
                        )
                        yield Checkbox(
                            "Action Failures",
                            value=self.config.get("notifications.action_failures", True),
                            id="notif-action-failures"
                        )
                        yield Checkbox(
                            "Security Alerts",
                            value=self.config.get("notifications.security_alerts", True),
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Select(
                            ...
                            value=str(self.config.get("notifications.check_interval", "10")),
            ...
            with TabPane("Advanced", id="advanced-tab"):
                with Container(id="advanced-settings", classes="settings-section"):
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("performance.monitoring_enabled", False),
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Input(
                            value=self.config.get("plugins.directory", "~/.github-cli/plugins"),
                            id="plugin-directory"
                        )
                    ...
                    with Horizontal(classes="setting-row"):
                        ...
                        yield Switch(
                            value=self.config.get("experimental.enabled", False),
    ...
    def save_settings(self) -> None:
        ...
        try:
            ...
            default_repo_view = cast(Select[str], self.query_one("#default-repo-view"))
            items_per_page = cast(Select[str], self.query_one("#items-per-page"))
            auto_refresh_interval = cast(Input, self.query_one("#auto-refresh-interval"))
            auto_refresh_enabled = cast(Switch, self.query_one("#auto-refresh-enabled"))
            ...
            show_line_numbers = cast(Switch, self.query_one("#show-line-numbers"))
            syntax_highlighting = cast(Switch, self.query_one("#syntax-highlighting"))
            ...
            respect_rate_limits = cast(Switch, self.query_one("#respect-rate-limits"))
            api_base_url = cast(Input, self.query_one("#api-base-url"))
            
            desktop_notifications = cast(Switch, self.query_one("#desktop-notifications"))
            notif_pr_updates = cast(Checkbox, self.query_one("#notif-pr-updates"))
            notif_issue_updates = cast(Checkbox, self.query_one("#notif-issue-updates"))
            notif_review_requests = cast(Checkbox, self.query_one("#notif-review-requests"))
            notif_action_failures = cast(Checkbox, self.query_one("#notif-action-failures"))
            notif_security_alerts = cast(Checkbox, self.query_one("#notif-security-alerts"))
            notification_interval = cast(Select[str], self.query_one("#notification-interval"))
            
            debug_mode = cast(Switch, self.query_one("#debug-mode"))
            log_level = cast(Select[str], self.query_one("#log-level"))
            performance_monitoring = cast(Switch, self.query_one("#performance-monitoring"))
            plugins_enabled = cast(Switch, self.query_one("#plugins-enabled"))
            plugin_directory = cast(Input, self.query_one("#plugin-directory"))
            experimental_features = cast(Switch, self.query_one("#experimental-features"))
            
            # General settings
            self.config.set("ui.default_repo_view", default_repo_view.value)
            if items_per_page.value != items_per_page.BLANK:
                self.config.set("ui.items_per_page", int(str(items_per_page.value)))
            self.config.set("ui.auto_refresh_interval", int(auto_refresh_interval.value))
            self.config.set("ui.auto_refresh_enabled", auto_refresh_enabled.value)
            ...
            self.config.set("ui.syntax_highlighting", syntax_highlighting.value)
            ...
            self.config.set("api.respect_rate_limits", respect_rate_limits.value)
            self.config.set("api.base_url", api_base_url.value)
            
            # Notification settings
            self.config.set("notifications.desktop_enabled", desktop_notifications.value)
            self.config.set("notifications.pr_updates", notif_pr_updates.value)
            self.config.set("notifications.issue_updates", notif_issue_updates.value)
            self.config.set("notifications.review_requests", notif_review_requests.value)
            self.config.set("notifications.action_failures", notif_action_failures.value)
            self.config.set("notifications.security_alerts", notif_security_alerts.value)
            if notification_interval.value != notification_interval.BLANK:
                self.config.set("notifications.check_interval", int(str(notification_interval.value)))
            
            # Advanced settings
            self.config.set("debug.enabled", debug_mode.value)
            self.config.set("debug.log_level", log_level.value)
            self.config.set("performance.monitoring_enabled", performance_monitoring.value)
            self.config.set("plugins.enabled", plugins_enabled.value)
            self.config.set("plugins.directory", plugin_directory.value)
            self.config.set("experimental.enabled", experimental_features.value)
    ...
    def import_settings(self) -> None:
        ...
        self.notify("Import settings functionality coming soon", severity="information")
    
    @on(Button.Pressed, "#export-settings")
    def export_settings(self) -> None:
        """Export settings to file."""
        self.notify("Export settings functionality coming soon", severity="information")
    ...
    def _refresh_ui_values(self) -> None:
        ...
        try:
            ...
            default_repo_view.value = self.config.get("ui.default_repo_view", "list")
            items_per_page.value = str(self.config.get("ui.items_per_page", "50"))
            auto_refresh_interval.value = str(self.config.get("ui.auto_refresh_interval", "60"))
            auto_refresh_enabled.value = self.config.get("ui.auto_refresh_enabled", True)
            ...
            syntax_highlighting.value = self.config.get("ui.syntax_highlighting", True)
...
def create_settings_widget(config: Config) -> SettingsWidget:
