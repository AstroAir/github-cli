...
SearchType = Literal["repositories", "code", "commits", "issues", "users", "topics"]
...
class IssueSearchResult(SearchResult):
    ...
    def subtitle(self) -> str:
        ...
        repo = extra.get('repository_url', '').split('/')[-1] if extra.get('repository_url') else 'Unknown'
...
class SearchManager:
    ...
    async def search(self, query: str, search_type: SearchType, results_table: DataTable) -> None:
        ...
        try:
            ...
            search_data = response.data if hasattr(response, 'data') else response
            
            # Extract results from response
            items = search_data.get('items', []) if isinstance(search_data, dict) else []
            
            # Convert to SearchResult objects
            self.search_results = self._convert_to_search_results(items, search_type)
            
            # Update table
            await self._update_table(results_table)
            
            total_count = search_data.get('total_count', len(items)) if isinstance(search_data, dict) else len(items)
            logger.info(f"Found {len(self.search_results)} results (total: {total_count})")
            results_table.app.notify(f"Found {len(self.search_results)} results", severity="information")
            ...
            results_table.app.notify(f"Unexpected error: {e}", severity="error")
        finally:
            self.loading = False
            ...
            try:
                match search_type:
                    ...
                    case "commits":
                        ...
                        result = SearchResult(
                            id=item.get('sha', 'unknown'),
                            title=commit_info.get('message', 'No message')[:100],
                    ...
                    case "topics":
                        result = SearchResult(
                            ...
                            title=item.get('display_name', item.get('name', 'Unknown')),
                    ...
                    case _:
                        result = SearchResult(
                            id=item.get('id', 0),
                            title=str(item.get('name', item.get('title', 'Unknown'))),
                            description=item.get('description'),
                            url=item.get('html_url', ''),
                            score=item.get('score'),
                            type=search_type,
...
class SearchWidget(Container):
    """Complete search functionality widget."""
    ...
    def compose(self):
        ...
        # Search help
        with Container(id="search-help"):
            yield Static("Search Tips:", classes="help-title")
            yield Static("�Use quotes for exact phrases: \"exact phrase\"", classes="help-item")
            yield Static("�Filter by language: language:python", classes="help-item")
            yield Static("�Filter by user: user:username", classes="help-item")
            yield Static("�Filter by repo: repo:owner/name", classes="help-item")
            yield Static("�Combine filters: language:python user:octocat", classes="help-item")
        
        # Search results
        with TabbedContent(id="search-results-tabs"):
            with TabPane("Results", id="results-tab"):
                results_table = DataTable(id="search-results-table", classes="search-results")
                results_table.add_columns("Title", "Description", "Score")
                yield results_table
            
            with TabPane("Advanced", id="advanced-tab"):
                yield Container(id="advanced-search")
    ...
    async def on_mount(self) -> None:
        ...
        # Hide search help initially (show when no results)
    ...
    def on_result_selected(self, event: DataTable.RowSelected) -> None:
        ...
        if event.row_key:
            result = self.search_manager.get_result_by_id(str(event.row_key.value))
            if result:
...
def create_search_widget(client: GitHubClient) -> SearchWidget:
