...
class RateLimitInfo(BaseModel):
    ...
    reset_time: int = Field(default=0, description="Reset time as Unix timestamp")
...
class GitHubClient:
    ...
    def _update_rate_limit(self, headers: dict[str, str]) -> None:
        ...
        try:
            ...
            if "X-RateLimit-Reset" in headers:
                self._rate_limit_info.reset_time = int(headers["X-RateLimit-Reset"])
    ...
    async def _request(
        ...
        try:
            ...
            if retry_count < self.MAX_RETRIES:
                ...
                logger.info(f"Retrying request after {delay}s (attempt {retry_count + 1}/{self.MAX_RETRIES})")
                await asyncio.sleep(delay)
                return await self._request(method, endpoint, params, data, headers, retry_count + 1)
                
            # Convert to our custom exception
            if isinstance(e, asyncio.TimeoutError):
                raise NetworkError(f"Request to {endpoint} timed out after {self.DEFAULT_TIMEOUT}s") from e
            else:
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse, endpoint: str) -> Any:
        ...
        try:
            ...
            # Handle client errors
                ...
                raise APIError(f"Validation error: {error_data}", status_code=422)
            ...
            # Handle other client errors
                ...
                error_data = await self._safe_json_parse(response)
                message = str(error_data.get("message", "Unknown API error")) if isinstance(error_data, dict) else "Unknown API error"
                logger.error(f"API error {response.status}: {message}")
                raise APIError(f"API error ({response.status}): {message}", status_code=response.status)
    ...
    async def paginated_request(
        ...
        params.setdefault("per_page", min(per_page, 100))  # GitHub API max is 100

        results: list[Any] = []
        current_page = params["page"]

        logger.debug(f"Starting paginated request for {endpoint} (max_pages={max_pages}, per_page={params['per_page']})")
            ...
            try:
                ...
                if isinstance(page_data, list):
                    if not page_data:  # Empty page means we're done
                        logger.debug(f"Reached empty page {page_num}, stopping pagination")
                        break
                        
                    results.extend(page_data)
                    logger.debug(f"Fetched page {page_num} with {len(page_data)} items")

                    # If we got fewer results than requested, we're done
                    if len(page_data) < params["per_page"]:
                        logger.debug(f"Partial page received ({len(page_data)} < {params['per_page']}), stopping pagination")
                        break
                else:
                ...
                if e.status_code == 404 and page_num > 1:
                    ...
                    logger.debug(f"404 on page {page_num}, assuming end of results")
                    break
                raise

        logger.info(f"Paginated request completed: {len(results)} total items from {page_num - current_page + 1} pages")
        return results
